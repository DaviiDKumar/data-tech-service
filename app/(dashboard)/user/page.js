...
import {
  ...
  IndianRupee, TrendingUp, SquareCheckBig, CheckCheck, ArrowUpRight, Clock
} from "lucide-react";
...
export default function UserDashboard() {
  ...
  const metrics = useMemo(() => {
    ...
    const earningsPerForm = 60;
    const totalEarnings = totalCompleted * earningsPerForm;
  ...
  // Countdown timer processing states
  ...
  const handleQuickStart = async () => {
    ...
    try {
      ...
      alert("A system error occurred while verifying assignments.");
    } finally {
    ...
    <div className={`min-h-screen pt-0 p-8 lg:p-8 lg:pt-0 ${robotoSlab.className}`}>
      <div className="max-w-6xl mx-auto space-y-12">
            ...
            <div className="flex items-center gap-3">
              <div className="text-left">
                <p className={`${passero.className} text-[18px] font-bold text-violet-700 leading-none`}>
                  User ID : <span className={` ${robotoSlab.className} text-[17px] font-bold text-black`}> {loginId || "N/A"}</span>
                </p>
        ...
        <div className="space-y-1">
          <p className="text-xs text-black/40 ml-2 mb-2 tracking-[0.3em] uppercase">Dashboard</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <button
            ...
            className="group relative flex cursor-pointer flex-col items-center justify-center p-6 h-20 shadow-2xl rounded-[2.5rem] transition-all bg-violet-600 text-white hover:bg-zinc-900 active:scale-95 shadow-slate-200 disabled:opacity-40"
          >
            <div className="flex justify-between w-full items-center">
              <p className="text-md font-black uppercase tracking-tighter leading-none">
                {isAssigning ? "Assigning..." : "Start Work"}
              </p>
              <ArrowUpRight size={20} className="bg-white text-violet-600 p-1 rounded-full group-hover:text-black" />
            </div>
          </button>

          <QuickAction href="/user/profile" label="Setup Profile" icon={<UserCircle size={20} />} />
          <QuickAction href="/user/reportuser" label="Check Report" icon={<BarChart3 size={20} />} />
          <QuickAction href="/user/queries" label="Queries" icon={<HelpCircle size={20} />} />
        </div>
        ...
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-black">Your Progress Towards Milestones</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
          <MilestoneCircle label="Tier 1 Goal" total={metrics.total} target={300} variant="violet" />
          <MilestoneCircle label="Tier 2 Goal" total={metrics.total} target={500} variant="red" />
          <MilestoneCircle label="Tier 3 Goal" total={metrics.total} target={700} variant="yellow" />
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={i} className={`${box.color} py-6 rounded-3xl flex flex-col items-center border-2 transition-all`}>
                <span className={`${passero.className} text-5xl font-black`}>{box.val}</span>
                <span className="text-[10px] font-black uppercase opacity-60 tracking-widest">{box.label}</span>
              </div>
            ))}
          </div>
...
function QuickAction({ href, label, icon, isBlack = false }) {
  ...
  return (
    <Link href={href} className={`flex items-center justify-between p-6 rounded-[2rem] transition-all duration-500 group ${isBlack ? blackStyle : whiteStyle}`}>
  ...
  return (
    <div className={`${themes[variant]} p-8 rounded-lg text-white shadow-xl flex flex-col h-40 relative overflow-hidden`}>
      <div className="flex justify-between items-start relative z-10">
        <span className="text-[10px] font-black uppercase tracking-widest opacity-80 leading-tight max-w-[130px]">{label}</span>
        <div className="bg-white/20 p-3 rounded-2xl backdrop-blur-md border border-white/10">{icon}</div>
      </div>
      <div className="mt-auto relative z-10">
        <h4 className={`${robotoSlab.className} text-4xl font-black leading-none tracking-tighter`}>{value}</h4>
        <p className="text-[9px] uppercase tracking-[0.2em] opacity-60 mt-2 font-bold">{subLabel}</p>
      </div>
  ...
  return (
    <div className="bg-white border-2 border-slate-200 shadow-2xl p-8 rounded-[3rem] flex flex-col items-center justify-center text-center relative overflow-hidden">
      ...
      <div className="space-y-1">
        ...
        <p className="text-[12px] font-bold text-black">{total} <span className="text-violet-600">/</span> {target}</p>
      </div>
...
function WorkJourneyGuide() {
  const steps = [
    { number: "1", title: "Account Registered", desc: "You have successfully registered and received work access.", variant: "blue" },
    { number: "2", title: "Start Working", desc: "Begin resume work and maintain accuracy while typing.", variant: "white" },
    { number: "3", title: "Submit & Review", desc: "Submit completed resumes. Quality team will review your work.", variant: "white" },
    { number: "4", title: "Payout Released", desc: "After approval, payout will be released to your account.", variant: "green" },
  ];
  ...
  const numberColors = { blue: "text-white/40", green: "text-white/40", white: "text-indigo-600" };

  return (
    <div className="mt-20 space-y-8">
      <div className="space-y-1">
        <h2 className="text-2xl font-black flex items-center gap-3 text-black">Work Journey Guide</h2>
        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Understand your complete workflow from start to payout.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {steps.map((step, i) => (
          <div key={i} className={`${themes[step.variant]} p-8 rounded-xl h-48 flex flex-col justify-between shadow-sm transition-transform hover:-translate-y-1 duration-300`}>
            <span className={`${robotoSlab.className} text-3xl leading-none ${numberColors[step.variant]}`}>{step.number}</span>
            <div className="space-y-2">
              <h3 className="font-black text-lg leading-tight uppercase tracking-tighter">{step.title}</h3>
              <p className="text-[11px] font-medium opacity-70 leading-relaxed">{step.desc}</p>
            </div>
          </div>
        ))}
      </div>
